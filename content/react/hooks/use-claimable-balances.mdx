---
title: useClaimableBalances
description: Fetch a paginated list of claimable balances in your React app using the Blux SDK.
---

import { Tab, Tabs } from 'fumadocs-ui/components/tabs';
import { Callout } from 'fumadocs-ui/components/callout';

The `useClaimableBalances` hook returns a paginated list of claimable balances. All parameters are optional — called with no arguments it returns the latest claimable balances from the network.

## Import
```tsx
import { useClaimableBalances } from "@bluxcc/react";
```
...
function App() {
  const { data } = useClaimableBalances({});
}
```
</Tab>
<Tab value="config.ts">
...
<Tab value="index.tsx">
```tsx
import { useClaimableBalances, Asset } from "@bluxcc/react";

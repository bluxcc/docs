...
The only required config is your **App ID** and a **network**. Everything else — appearance, login methods, language, wallets — has defaults.
...
```tsx
import { networks, BluxProvider, useBlux } from "@bluxcc/react";
...
```ts
import { createConfig, core, blux } from "@bluxcc/core";

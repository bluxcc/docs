...
3. **Initialize Blux** with `createConfig` / `BluxProvider`, passing your App ID and a network.
...
Wrap your app with `BluxProvider` (React) or call `createConfig` once at startup (vanilla JS). The only required options are `appId` and `networks`.
